...
public class UnbundleFD : MonoBehaviour {
    ...
    // Bundle Id list
    int[] bundleId;
    Dictionary<int, int> bundleDict = new ();
    ...
    private IEnumerable<GameObject> Displayables;
    public bool ManageDisplayers = false;

    // Use this for initialization
    private void Start () 
    {
        Displayer = GameObject.FindGameObjectWithTag("Tablet").transform;
        Displayables = GameObject.FindGameObjectsWithTag("Container");
    ...
    public void InitUnbundling(List<KeyValuePair<GameObject, GameObject>> listGO, bool drawing)
    {
        ...
        {
            GameObject go = kv.Key; //Referent
            GameObject sensorVisu = kv.Value; //Mark

            List<Vector3> line = new ();
            //Debug.Log("Use of the KeyValue Pair");

            //Vector3 goP = go.transform.TransformPoint(new Vector3(0, 0.1f, 0));
    ...
    {
        if (!ManageDisplayers) return;

        foreach (var displayable in Displayables.Where(d => d.activeSelf))
        {
            bool display = Vector3.Angle(Displayer.forward * -1, displayable.transform.position - Displayer.position) < FacingAngle;
            DisplayLink(display);
        }
    }
    ...
    private Vector3[] ComputeDisplacementFromViewport(Matrix4x4 trans)
    {
        ...
        foreach (KeyValuePair<GameObject, GameObject> kv in pointsList)
        {
            ...
            //lineTab[lineLenght * count] = go.transform.TransformPoint(new Vector3(0, 0.1f, 0));
    ...
    private void ApplyDisplacement()
    {
        ...
        shader.Dispatch(dispKernel, lineNb, 1, 1);//dispKernel
    }
    ...
    private void DrawTube()
    {
        if (LineRendererDrawing)
        {
            ...
            foreach (KeyValuePair<GameObject, GameObject> kv in pointsList)
            {
                ...
                lr.startWidth = radiusTube;
                lr.endWidth = radiusTube;
    ...
    private void RedrawTube()
    {
        if (LineRendererDrawing) 
        {
            ...
            foreach (KeyValuePair<int, LineRenderer> entry in tubeList)
            {
                ...
                {
                    pointsTube[i] = lineTab[idBegining + i];//getAvgPos(i);
                ...
                linkMat.color = colorT;
                matArray[0] = linkMat;
                entry.Value.materials = matArray;
    ...
    private void DisplayLink (bool display)
    {
        foreach (var entry in tubeList)
            entry.Value.enabled = display;
    }

    public void SetManageDisplayer(bool active)
    {
        ManageDisplayers = active;
    }
