...
namespace BrushingAndLinking
{
    public class FilterSliderManager : MonoBehaviour
    {
        ...
        private void AdjustSliderText()
        {
            ...
            if (ApproximatelyEqualEpsilon(XMinSlider.position.x, xMinRange))
            {
                XMinTextFront.text = "";
                XMinTextBack.text = "";
            }
            if (ApproximatelyEqualEpsilon(XMaxSlider.position.x, xMaxRange))
            {
                XMaxTextFront.text = "";
                XMaxTextBack.text = "";
            }
            if (ApproximatelyEqualEpsilon(YMinSlider.position.x, yMinRange))
            {
                YMinTextFront.text = "";
                YMinTextBack.text = "";
            }
            if (ApproximatelyEqualEpsilon(YMaxSlider.position.x, yMaxRange))
            {
                YMaxTextFront.text = "";
                YMaxTextBack.text = "";
            }
        ...
        private void AdjustSliderRangeGuidesAndThresholds()
        {
            if (!StudyManager.Instance.TrialActive || (StudyManager.Instance.TrialActive && StudyManager.Instance.CurrentTrial.Task == TaskType.Tutorial))
            {
                ...
                var labels = axis.GetComponentsInChildren<Transform>()
                    .Where(t => t.gameObject.name == "TickLabel")
                    .Select(t => t.GetComponent<TextMesh>());
                ...
                labels = axis.GetComponentsInChildren<Transform>()
                    .Where(t => t.gameObject.name == "TickLabel")
                    .Select(t => t.GetComponent<TextMesh>());
